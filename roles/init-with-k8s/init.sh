...
echo "安装初始化需要的工具，不需要输出"
yum install -y wget net-tools telnet vim git ntpdate > /dev/null
...
echo "安装zsh插件"
cd /root/.oh-my-zsh/custom/plugins/ && source /etc/profile && pon && git clone https://github.com/zsh-users/zsh-syntax-highlighting.git > /dev/null
cd /root/.oh-my-zsh/custom/plugins/ && source /etc/profile && pon && git clone https://github.com/zsh-users/zsh-autosuggestions.git > /dev/null
